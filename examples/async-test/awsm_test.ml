...
module Ecs = struct
  module Values = Awsm_ecs_async.Values
  module Io = Awsm_ecs_async.Io

  let call = Awsm_async.Http.Io.call ~service:Values.service
end

module Ecr = struct
  module Values = Awsm_ecr_async.Values
  module Io = Awsm_ecr_async.Io

  let call = Awsm_async.Http.Io.call ~service:Values.service
end

module S3 = struct
  module Values = Awsm_s3_async.Values
  module Io = Awsm_s3_async.Io
...
let suite_main ~sso bucket () =
  let%bind cfg =
    match sso with
    | true -> Awsm_sso_async.Util.Cfg.get_exn ()
    | false -> Awsm_async.Cfg.get_exn ()
  in
  let%bind () =
    pr "=== EC2 ===";
    dispatch_exn
